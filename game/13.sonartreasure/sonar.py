...
def isValidMove(x, y):
    # Return True if the coordinate are on the board, otherwise False.
...
def makeMove(board, chests, x, y):
    ...
    for cx, cy in chests:
        ...
        if distance < smallestDistance:  # we want the closest treature chest
...
# if input().lower().startswith('y'):  # v3.x
if raw_input().lower().startswith('y'):  # v2.x
    showInstructions()
